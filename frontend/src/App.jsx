...
import { demoResults, demoSessions, loadResults, login } from './api'
...
function App() {
  ...
  const signIn = async (email, password) => {
    try {
      ...
      setLoginError(error.message || 'We couldn\'t sign you in. Please check the demo account instead.')
    }
  }

  if (!student) return <Login onSignIn={signIn} error={loginError} />
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          <Announcement />
          {activeView === 'Overview' && <Overview student={student} results={results} goToResults={() => setActiveView('My results')} setActiveView={setActiveView} />}
...
function Overview({ student, results, goToResults, setActiveView }) {
  const stats = useMemo(() => {
    const totalCredits = results.reduce((total, row) => total + row.credits, 0)
    const weightedPoints = results.reduce((total, row) => total + (row.points || 0) * row.credits, 0)

    return {
      credits: totalCredits,
      gpa: totalCredits ? (weightedPoints / totalCredits).toFixed(2) : '0.00',
    }
  }, [results])
  return <><div className="page-heading"><div><span className="section-kicker">Monday, 17 September 2026</span><h1>Good morning, {student.firstName || 'Amara'} <span className="wave">✦</span></h1><p>Here is your academic snapshot for the current session.</p></div><button className="accent-button" onClick={() => window.print()}><Download size={17} /> Download result</button></div><section className="stat-grid" aria-label="Academic summary"><Stat label="Current CGPA" value={stats.gpa} meta="out of 5.00" icon={Sparkles} accent="blue" trend="+0.18" /><Stat label="Credits completed" value={String(stats.credits || 0)} meta="of 120 credits" icon={GraduationCap} accent="teal" trend="65%" /><Stat label="Academic status" value="Good standing" meta="No outstanding alerts" icon={CheckCircle2} accent="green" /><Stat label="Current semester" value="300 level" meta="First semester · 2025/26" icon={BookOpen} accent="orange" /></section><div className="dashboard-grid"><section className="panel results-preview"><PanelHeader title="Latest results" action="View all results" onAction={goToResults} /><div className="table-scroll"><ResultTable rows={results.slice(0, 4)} /></div><div className="panel-foot"><span>Showing your most recent published grades</span><button onClick={goToResults}>See full results <ArrowUpRight size={15} /></button></div></section><section className="panel progress-panel"><PanelHeader title="Degree progress" /><div className="progress-ring"><div><strong>65%</strong><span>complete</span></div></div><div className="progress-details"><div><span>Credits completed</span><strong>{stats.credits || 0} / 120</strong></div><div className="progress-bar"><span /></div><p><strong>{Math.max(0, 120 - (stats.credits || 0))} credits</strong> remaining to graduation</p></div><div className="graduation-note"><GraduationCap size={18} /><span>Estimated graduation<br /><strong>June 2027</strong></span></div></section></div><section className="quick-actions"><span className="section-kicker">Quick actions</span><button onClick={() => window.print()}><Download size={18} /><span><strong>Download result</strong><small>Save a PDF copy of your grades</small></span><ArrowUpRight size={16} /></button><button className="secondary-button" onClick={() => setActiveView('Transcript request')}><FileText size={18} /><span><strong>Request transcript</strong><small>Order an official academic record</small></span><ArrowUpRight size={16} /></button></section></>
}
...
function ResultsView({ results }) {
  ...
  const weightedPoints = results.reduce((total, row) => total + (row.points || 0) * row.credits, 0)
  const gpa = credits ? (weightedPoints / credits).toFixed(2) : '0.00'
